...
const TaskItemOptimistic = ({ id, defaultValue, isChecked }) => {
  ...
  const handleInputChange = (event) => {
    ...
    updateTask({
      variables: { id, name: event.target.value, completed },
      optimisticResponse: {
        __typename: "Mutation",
        updateTask: {
          __typename: "Task",
  ...
  const handleCompletedChange = (event) => {
    ...
    updateTask({
      variables: { id, name, completed: event.target.checked },
      optimisticResponse: {
        __typename: "Mutation",
        updateTask: {
          __typename: "Task",
  ...
  return (
    ...
    >
      <input
        style={{ border: `${name.length === 0 ? "1px solid red" : "1px solid #ccc"}`, padding: "8px", fontSize: "16px" }}
        value={name}
...
export default TaskItemOptimistic;
